...
def createCNN(n_target_bits,
    ...
    if n_layers == 2:
        n_bits_per_channel = (conv_kernel_size ** 2.0 * n_input_channels + input_image_size_1 ** 2.0 * n_classes) * n_bits_per_weight + n_bits_per_batchnorm
        n_channels_real = (n_target_bits - n_classes * n_bits_per_bias) / n_bits_per_channel
        layout = [max(round(n_channels_real), 1), n_classes]
    elif n_layers == 3:
        a = n_bits_per_weight * 2.0 * conv_kernel_size ** 2.0
        b = n_bits_per_weight * (conv_kernel_size ** 2.0 * n_input_channels + input_image_size_2 ** 2.0 * 2.0 * n_classes) + 3.0 * n_bits_per_batchnorm
...
def run(dataset,
    ...
    #-----------------------------------------------------------------------------------------------------------------------
    # Create the model
    model = createCNN(n_target_bits=pgm_n_parameters * pgm_bits_per_parameter,
                      n_classes=n_classes,
                      n_bits_per_weight=n_bits_per_weight,
                      n_layers=n_layers,
...
def main():
    ...
    if activation == 'relu':
        rng = np.random.RandomState(seed=730205982)
    elif activation == 'sign':
        rng = np.random.RandomState(seed=974751368)
    else:
        raise NotImplementedError('Activation \'{}\' not implemented'.format(activation))
    
    gridvals_pgm_bits_per_parameter = list(range(1, 5))
