...
from BNCModelsCommons import compute_chow_liu_structure
...
def run(dataset,
    ...
    #-----------------------------------------------------------------------------------------------------------------------
    # Compute Chow Liu TAN Structure
    # The resulting structures are different based on the root_feature_idx but all of them can represent the same
    # distributions. Hence, we can select an arbitrary index without restriction.
    root_feature_idx = 0
    ...
    augmenting_features = compute_chow_liu_structure(mi_graph, root_feature_idx)
    ...
    stats['augmenting_features'] = augmenting_features
    return stats
...
def main():
    ...
    gridvals_random_params = list(range(600))
    ...
    rng_experiment014A = np.random.RandomState(seed=265304870)
    rng_seeds = rng_experiment014A.randint(1, 1e9, size=(1000,))
    random_bnc_hybrid_tradeoff_014A = 10.0 ** rng_experiment014A.uniform(low=0.0, high=3.0, size=(500,))
    random_bnc_hybrid_gamma_014A = 10.0 ** rng_experiment014A.uniform(low=-1.0, high=2.0, size=(500,))
    random_bnc_hybrid_eta_014A = rng_experiment014A.uniform(low=1.0, high=20.0, size=(500,))
    rng_seeds = np.concatenate([rng_seeds, rng_experiment014A.randint(1, 1e9, size=(n_jobs - rng_seeds.size,))])

    # Generate the same random hyperparameters as in experiment015A/B
    rng_experiment015A = np.random.RandomState(seed=361273804)
    rng_experiment015A.randint(1, 1e9, size=(9600,))
    random_bnc_hybrid_tradeoff_015A = 10.0 ** rng_experiment015A.uniform(low=1.0, high=3.0, size=(100,))
    random_bnc_hybrid_gamma_015A = 10.0 ** rng_experiment015A.uniform(low=-1.0, high=2.0, size=(100,))
    random_bnc_hybrid_eta_015A = np.full((100,), 10.0, dtype=np.float32)

    # Concatenate random parameters. The first 500 are the same as in experiment014A. The next 100 are the same as in experiment015A.
    random_bnc_hybrid_tradeoff = np.concatenate([random_bnc_hybrid_tradeoff_014A, random_bnc_hybrid_tradeoff_015A], axis=0)
    random_bnc_hybrid_gamma = np.concatenate([random_bnc_hybrid_gamma_014A, random_bnc_hybrid_gamma_015A], axis=0)
    random_bnc_hybrid_eta = np.concatenate([random_bnc_hybrid_eta_014A, random_bnc_hybrid_eta_015A], axis=0)
