...
from BNCModels import TanBayesNetClassifierSubset
...
def run(dataset,
    ...
    #-----------------------------------------------------------------------------------------------------------------------
    # Create the model
    model = TanBayesNetClassifierSubset(
            n_classes,
            n_unique_features,
            augmenting_features,
...
def get_feature_permutation_row_wise(n_features):
    return np.arange(n_features)


def get_feature_permutation_row_wise_back_and_forth(n_features):
    img_width = int(n_features ** 0.5)
    feature_idx_list = []
    for row_idx in range(img_width):
        if row_idx % 2 == 0:
            feature_idx_list.append(np.arange(img_width) + img_width * row_idx)
        else:
            feature_idx_list.append(np.arange(img_width - 1, -1, -1) + img_width * row_idx)
    return np.concatenate(feature_idx_list)


def get_feature_permutation_main_diagonal(n_features):
    img_width = int(n_features ** 0.5)
    feature_idx_list = []
    for row_idx in range(img_width):
        for col_idx in range(row_idx):
            feature_idx_list += np.ravel_multi_index(([row_idx, col_idx], [col_idx, row_idx]), (img_width, img_width)).tolist()
        feature_idx_list += np.ravel_multi_index(([row_idx], [row_idx]), (img_width, img_width)).tolist()
    return np.asarray(feature_idx_list)


def get_feature_permutation_center_outwards(n_features):
    img_width = int(n_features ** 0.5)

    feature_idx_list = []
    if img_width % 2 == 0:
        idx_lo = img_width // 2 - 1
        idx_hi = img_width // 2
    else:
        idx_lo = img_width // 2 - 1
        idx_hi = img_width // 2 + 1
        feature_idx_list += np.ravel_multi_index(([img_width // 2], [img_width // 2]), (img_width, img_width)).tolist()

    for idx in range(img_width // 2):
        feature_idx_list += np.ravel_multi_index((np.full(idx_hi-idx_lo-1, idx_lo), np.arange(idx_lo+1, idx_hi)), (img_width, img_width)).tolist()
        feature_idx_list += np.ravel_multi_index((np.full(idx_hi-idx_lo-1, idx_hi), np.arange(idx_lo+1, idx_hi)), (img_width, img_width)).tolist()
        feature_idx_list += np.ravel_multi_index((np.arange(idx_lo+1, idx_hi), np.full(idx_hi-idx_lo-1, idx_lo)), (img_width, img_width)).tolist()
        feature_idx_list += np.ravel_multi_index((np.arange(idx_lo+1, idx_hi), np.full(idx_hi-idx_lo-1, idx_hi)), (img_width, img_width)).tolist()
        feature_idx_list += np.ravel_multi_index(([idx_lo, idx_lo, idx_hi, idx_hi], [idx_lo, idx_hi, idx_lo, idx_hi]), (img_width, img_width)).tolist()
        idx_lo -= 1
        idx_hi += 1
    return np.asarray(feature_idx_list)


def get_feature_configuration(feature_configuration_idx, n_features, img_transposed=False):
    assert int(float(n_features) ** 0.5) ** 2.0 == n_features
    img_width = int(n_features ** 0.5)
    if feature_configuration_idx == 0:
        feature_permutation = get_feature_permutation_row_wise(n_features)
    elif feature_configuration_idx == 1:
        feature_permutation = get_feature_permutation_row_wise_back_and_forth(n_features)
    elif feature_configuration_idx == 2:
        feature_permutation = get_feature_permutation_main_diagonal(n_features)
    elif feature_configuration_idx == 3:
        feature_permutation = get_feature_permutation_center_outwards(n_features)
    else:
        raise NotImplementedError('Feature configuration {} not implemented'.format(feature_configuration_idx))
    assert feature_permutation.size == n_features
    assert np.unique(feature_permutation).size == n_features

    if img_transposed:
        # feature_permutation is computed by assuming that images are stored row-wise. If this is not the case, correct for it now.
        # mnist and usps are stored in columnwise order (verified by plotting)
        # first transpose permutation, then apply feature transformation
        transpose_permutation = np.reshape(np.arange(n_features), (img_width, img_width)).T.flatten()
        feature_permutation = transpose_permutation[feature_permutation]

    # Build squared distance graph for pixels
    mesh_col, mesh_row = np.meshgrid(np.arange(img_width, dtype=np.float32), np.arange(img_width, dtype=np.float32))
    dist = (np.reshape(mesh_col, (-1,)) - np.reshape(mesh_col, (-1, 1))) ** 2.0 # note: dist is symmetric --> works independent of rows and columns
    dist = dist + (np.reshape(mesh_row, (-1,)) - np.reshape(mesh_row, (-1, 1))) ** 2.0
    dist = dist[feature_permutation[:, None], feature_permutation] # permute distances according to feature_permutation

    # augmenting_feature_permutation is wrt the already permuted data
    augmenting_feature_permutation = np.full((n_features, n_features), -1, np.int32)
    for feature_idx in range(n_features):
        feature_dists = dist[feature_idx, :feature_idx]
        sort_idx = np.argsort(feature_dists)
        augmenting_feature_permutation[feature_idx, :feature_idx] = sort_idx

    return feature_permutation, augmenting_feature_permutation
...
def main():
    ...
    gridvals_feature_configuration = list(range(4))
    gridvals_max_augmenting_features = [2, 3, 6, 9]
    gridvals_random_params = list(range(100))
    gridvals_learning_rate_start = [3e-2, 3e-3]
    gridvals = [gridvals_feature_configuration, # feature_configuration must be the first one so that we can add new configurations easily [no problems with taskid]
                gridvals_max_augmenting_features,
    ...
    feature_configuration_idx = params[0]
    max_augmenting_features = params[1]
    ...
    feature_permutation, augmenting_feature_permutation = get_feature_configuration(feature_configuration_idx, n_features[dataset])
    ...
    for feature_idx in range(n_features[dataset]):
        if feature_idx + 1 <= max_augmenting_features:
            augmenting_features_onehot[feature_idx, :feature_idx+1] = 1
        else:
            augperm = augmenting_feature_permutation[feature_idx, :feature_idx]
            augmenting_features_onehot[feature_idx, augperm[:max_augmenting_features-1]] = 1
            augmenting_features_onehot[feature_idx, feature_idx] = 1
        augmenting_features.append(np.nonzero(augmenting_features_onehot[feature_idx])[0].tolist())
    ...
    print('feature_configuration_idx: {}'.format(feature_configuration_idx))
    print('feature_permutation:')
    print(feature_permutation)
    print('possible augmenting features [max_augmenting_features: {}]:'.format(max_augmenting_features))
    for feature_idx in range(n_features[dataset]):
    ...
    stats_stacked['experiment_parameters/feature_configuration_idx'] = feature_configuration_idx
